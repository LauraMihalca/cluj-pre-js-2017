(function () {
    ...
    const DropDownSelector = function (options = {}, legend = '', opts = []) {
    ...
    const Fieldset = function (options = {}) {
        const selectArray = options.dropDownOptions.map(el => `<li>${DropDownSelector(el, options.legend, options.options)}</li>`);
        return `
    <fieldset>
                ...
                this.markup = `
                    ...
                    <div class="new-eval">
                        <form id="new_eval_form">
                            ...
                            ${TechnicalLevelPicker(feedbackApp.data.newEvaluation.technicalLevelItems())}
                            ${TextareaSection(feedbackApp.data.newEvaluation.textareaItems())}
    ...
    feedbackApp.newEvaluation.view = {
        setup: new NewEvaluationPage(),
    };
}());
